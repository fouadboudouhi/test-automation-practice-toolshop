SHELL := /bin/bash

COMPOSE_FILE ?= docker/docker-compose.yml
PROFILE      ?=   # empty = CI/default stack; set PROFILE=local for local extras

API_SERVICE  ?= laravel-api
DB_SERVICE   ?= mariadb

BASE_URL     ?= http://localhost:4200
SEED_CMD     ?= php artisan migrate:fresh --seed

TEST_ROOT    ?= ui-tests
SMOKE_TAG    ?= smoke
REG_TAG      ?= regression

ARTIFACTS    ?= artifacts_local

DC := docker compose -f $(COMPOSE_FILE)
DC_UP := $(DC) $(if $(PROFILE),--profile $(PROFILE),)

.PHONY: help up down clean logs ps wait-api wait-db seed verify-seed rfbrowser-init smoke regression test-all

help:
	@echo "Targets:"
	@echo "  make up PROFILE=local  - start stack + local extras (web/cron)"
	@echo "  make up                - start minimal stack (CI-compatible)"
	@echo "  make down              - stop stack (keep volumes)"
	@echo "  make clean             - stop stack and remove volumes"
	@echo "  make seed              - wait db + migrate:fresh --seed + verify"
	@echo "  make smoke             - run smoke tests (tagged)"
	@echo "  make regression        - run regression tests (tagged)"
	@echo "  make test-all          - up -> seed -> smoke (gate) -> regression"
	@echo "  make logs              - tail stack logs"
	@echo "  make ps                - show stack status"

up:
	$(DC_UP) up -d --pull missing
	$(DC_UP) ps

down:
	$(DC_UP) down

clean:
	$(DC_UP) down -v

ps:
	$(DC_UP) ps

logs:
	$(DC_UP) logs --no-color --tail=200

wait-api:
	@echo "Waiting for API (internal healthcheck via $(API_SERVICE)) ..."
	@for i in {1..120}; do \
		$(DC_UP) exec -T $(API_SERVICE) php -r 'exit(@file_get_contents("http://127.0.0.1/api/documentation") ? 0 : 1);' >/dev/null 2>&1 && echo "API reachable" && exit 0; \
		sleep 2; \
	done; \
	echo "API not reachable"; \
	$(DC_UP) logs --no-color --tail=300; \
	exit 1

wait-db:
	@echo "Waiting for DB $(DB_SERVICE) ..."
	@for i in {1..60}; do \
		$(DC_UP) exec -T $(DB_SERVICE) sh -lc 'mysqladmin ping -h 127.0.0.1 -uroot -p"$$MYSQL_ROOT_PASSWORD" --silent' >/dev/null 2>&1 && echo "DB ready" && exit 0; \
		sleep 2; \
	done; \
	echo "DB not ready"; \
	$(DC_UP) logs --no-color --tail=300; \
	exit 1

seed: wait-api wait-db
	@echo "Running seed: $(SEED_CMD)"
	$(DC_UP) exec -T $(API_SERVICE) $(SEED_CMD)
	$(MAKE) verify-seed

verify-seed:
	@echo "Verifying product count > 0 ..."
	@OUT="$$( $(DC_UP) exec -T $(API_SERVICE) php artisan tinker --execute='echo \App\Models\Product::count();' 2>/dev/null || true )"; \
	COUNT="$$(echo "$$OUT" | tr -dc '0-9')"; \
	echo "Product count: $$COUNT"; \
	if [[ -z "$$COUNT" || "$$COUNT" -le 0 ]]; then \
		echo "Seed verification failed."; \
		echo "$$OUT"; \
		exit 1; \
	fi

rfbrowser-init:
	rfbrowser init

smoke: rfbrowser-init
	BASE_URL="$(BASE_URL)" robot --outputdir "$(ARTIFACTS)/smoke" --include "$(SMOKE_TAG)" "$(TEST_ROOT)"

regression: rfbrowser-init
	BASE_URL="$(BASE_URL)" robot --outputdir "$(ARTIFACTS)/regression" --include "$(REG_TAG)" "$(TEST_ROOT)"

test-all: up seed smoke regression